...
public class QueryStringAuthGenerator {
    ...
    public QueryStringAuthGenerator(String awsAccessKeyId, String awsSecretAccessKey, 
                                    boolean isSecure)
    {
        this(awsAccessKeyId, awsSecretAccessKey, null, isSecure, Utils.DEFAULT_HOST);
    }

    public QueryStringAuthGenerator(String awsAccessKeyId, String awsSecretAccessKey,
                                    List<String> tokens, boolean isSecure, String server)
    {
        this(awsAccessKeyId, awsSecretAccessKey, tokens, isSecure, server,
             isSecure ? Utils.SECURE_PORT : Utils.INSECURE_PORT);
    }

    public QueryStringAuthGenerator(String awsAccessKeyId, String awsSecretAccessKey, 
                                    List<String> tokens, boolean isSecure, String server, int port)
    {
        this(awsAccessKeyId, awsSecretAccessKey, tokens, isSecure, server,
    ...
    {
        this(awsAccessKeyId, awsSecretAccessKey, null, isSecure, server, 
             isSecure ? Utils.SECURE_PORT : Utils.INSECURE_PORT, 
             callingFormat);
    }
                             ...
                             CallingFormat callingFormat)
    {
        ...
        //PLEASE NOTE - While it is possible to use both a User and Product token to generate
        //a signed URL, it is HIGHLY RECOMMENDED that the UserToken alone, from a hosted / web style
        //activation, is used to create signed urls for security reasons.  
        //Please see README for further detail
    ...
    private String generateURL(String method, String bucketName, String key, Map pathArgs, Map headers) 
    {
        ...
        if (tokensString_ != null) {
            if (headers == null) { headers = new HashMap(); }
            ArrayList tokenList = new ArrayList();
            tokenList.add(tokensString_);
            headers.put(XAMZ_SECURITY_TOKEN, tokenList);
        }
        ...
        if (tokensString_ != null) {
            pathArgs.put(XAMZ_SECURITY_TOKEN, Utils.urlencode(tokensString_));
        }
